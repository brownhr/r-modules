...
```{r exampleplots, eval=T, include=F}
example.1 <-
  ggplot(NC, aes(y = TOTJOB2000, x = MNEM2000)) +
  geom_point(aes(size = MNEM2000 %>% log10() %>% sqrt()),
             color = "steelblue",
             alpha = .3,
             show.legend = F) +
...
example.2 <-
  ggplot(NC %>% st_transform(st_crs(2264))) +
  geom_sf(lwd = .5,
          color = "gray30",
          fill = 'transparent') +
  scale_color_continuous(type = "viridis", n.breaks = 10) +
  theme_minimal() +
  theme(
...
```{r cowplot2, message=FALSE, warning=FALSE, include=TRUE}
cowplot::plot_grid(
