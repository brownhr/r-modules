...
Let's select the columns and work with our data by creating a new `object`, "`survey_sub`". Using `%>%`, we'll pipe our `survey` into the `dplyr::select()` function.  
...
Our salaries don't appear normally distributed, do they? When exploring data, it's often a good idea to *transform* your data to help visualize the general distribution of the data. We'll transform by taking the base-10 logarithm of our salary variable:
