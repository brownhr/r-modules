...
```{r select}
survey_sub <- survey %>%
  select(`Survey Year`, Country, PrimaryDatabase, SalaryUSD, YearsWithThisDatabase, YearsWithThisTypeOfJob, Education, Gender)
```
...
survey_sub <- survey_sub %>%
  filter(
    ...
    !(Gender %in% c("Not Asked", "Prefer not to say"))
  )
```
...
Let's perform some final cleaning, and pull out only the variables we're interested in:
...
survey_clean = survey_sub %>% 
  select(
    -PrimaryDatabase, -Country
  ) %>% 
  filter(
    # Unfortunately, there were only 4 NB respondents, so we'll have to stick
    # with a Male/Female binary so that our n is still greater than 30.
    Gender %in% c("Male", "Female"),
    
    # Same for education; because there are only 13 remaining PhD respondents,
    # we shouldn't try to make any statistical inferrences.
