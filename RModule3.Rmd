...
pois <- 0:30 %>%
  ...
  `colnames<-`("Value") %>% 
  mutate(
    Occurrances = dpois(Value, lambda = 2.5)
  )



ggplot(pois, aes(x = Value, y = Occurrances)) + 
  geom_col(
    fill = "steelblue"
  ) + 
  labs(
...
Continuous distributions differ from discrete distributions in that outcomes can lie on a continuous range of values, for example, average daily temperature, atmospheric pressure, or elevation of state capitols. 
