...
Describe a bit of choropleth mapping here; classes, quantiles, etc. 
...
# poiscol <- function(.df, n) {
#   name <- paste("pois_", n)
#   mutate(.df, !!name := dpois(Occurances, n))
# }
# 
# for (i in seq(1, 6, 1)) {
#   poiscol(num_occurances, i)
# }
...
bscols(
  plot_ly(
    data = num_occurances,
    x = ~ Occurances,
    y = ~ (dpois(Occurances, lambda = 2.5) * 100 %>% round(digits = 0)),
    hoverinfo = "text",
    text = ~ paste((dpois(Occurances, lambda = 2.5) * 100) %>% round(digits = 2), "%")
  ) %>%
    add_bars() %>%
    layout(
      xaxis = list(title = "Number of Occurances"),
      yaxis = list(title = "Percent Chance")
    )
)
```
...
<!--
```{r leaflet, eval = F, echo = F, include = F, message=F, warning=F}
library(leaflet)
shared_nc <- SharedData$new(NC_Counties %>%
  mutate(NAME = str_remove(NAME, ", North Carolina")) %>%
  arrange(Population))
bscols(
  leaflet(shared_nc, width = "100%", height = 300) %>%
    addTiles() %>%
    addPolygons(
      color = "gray40", weight = 1, smoothFactor = 0.5,
      opacity = 1, fillOpacity = 0.9,
      fillColor = ~ colorQuantile("Blues", Population)(Population),
  ...
  plot_ly(
    ...
    layout(
      xaxis = list(title = "County Name"),
      yaxis = list(title = "Total Population (Log10)"),
